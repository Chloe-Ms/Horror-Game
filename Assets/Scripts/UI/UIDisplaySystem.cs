...
public class UIDisplaySystem : MonoBehaviour
{
    ...
    private void ActivateDeactivate(EInteractable enumInteract, bool activate)
    {
        switch (enumInteract)
        {
            case EInteractable.BLOOD:
                _textUIBlood?.SetActive(activate);
                break;

            case EInteractable.MIXING_TABLE:
            case EInteractable.OBJECT:
            case EInteractable.OBJECT_OBJECTIVE:
